  ...
  <div class="photo-wall-wapper">
    ...
    <el-dialog title="详细信息" :visible.sync="dialogFormVisible" width="70%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-popconfirm confirmButtonText="确认" cancelButtonText="手抖了" icon="el-icon-warning" iconColor="red"
          title="确定删除该图片吗？" slot="button" @onConfirm="deletePhoto()">
    ...
    <div class="scroll-wapper" ref="scroll">
      <div class="scroll-content">
        <album-card v-for="item in imageInfo" :key="item.id"
          style="margin-bottom: 5%; margin-right: 1.3%; margin-left: 1.3%; width: 30%; height:20%">
          <el-image :fit="'cover'" :src="item.url" @load="onLoadSuccess" @error="onLoadError" slot="image" class="image"
            :preview-src-list="imageUrls"></el-image>
          <i slot="describe" class="el-icon-more more-btn" @click="showImageInfo(item)"></i>
          <i v-if="canEdit" slot="button" class="el-icon-magic-stick edit-btn" @click="toEdit(item)"></i>
        </album-card>
      </div>
    </div>
        ...
        if (this.canEdit) {
          ...
          axios.post('user/delete-image/', form, {
            ...
            if (error_code === 0) {
              ...
              this.$store.commit('updateAlbumCover', result.data.album_cover_url)
